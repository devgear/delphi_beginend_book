...
{
  if(typeof convertStringToBase64 == 'function') {
...
function getSessionID()
{
  ...
  if ($$SessionID$$ != null) {
    ...
    if ($$SessionExpires$$ != null && (new Date()) >= $$SessionExpires$$) {
      ...
      $$SessionID$$ = null;
      $$SessionExpires$$ = null;
      if(typeof notifySessionExpired == 'function') {
...
function initSessionData(useSessionCookie)
{
  $$SessionID$$ = null;
  $$SessionExpires$$ = null;
  ...
  if ($$USE_SESSION_COOKIE$$ === true && navigator.cookieEnabled) {
    $$SessionID$$ = getCookie("dssessionid");
  }
}
...
function setSessionData(sessionId, expireDate)
{
  ...
  if ($$USE_SESSION_COOKIE$$ === true && navigator.cookieEnabled) {
    if (sessionId != null) {
      setCookie("dssessionid", sessionId, expireDate);
    } else {
      deleteCookie("dssessionid");
    }
  }
}
